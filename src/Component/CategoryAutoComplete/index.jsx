...
export const CategoryAutoComplete = ({
  ...
  const handleCategoryKeyDown = data => {
    if (onKeyDown) onKeyDown({keyCode: data, value: val})
  }

  const handleInputBlur = data => {
    onBlur({value: data.target.value})
  ...
  return (
    <StyledCategoryAutoComplete {...props}>
      <CategoryInput
        ...
        onClick={!onDisabled && handleInputFocus}
        onIconClick={() => {
          handleInputChange('')
