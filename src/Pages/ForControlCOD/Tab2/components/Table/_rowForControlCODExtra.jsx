...
import { ForControlCOD_TABLE_ROW_EXTRA_TABS } from 'Pages/ForControlCOD/Tab2/interfaces/_constants'
import { useState } from 'react'
import styled from 'styled-components'
import { RowTabDetailForControlCOD } from './_rowTabDetailForControlCOD'
import { RowTabDetailOrder } from './_rowTabDetailOrder'
import { RowTabHistory } from './_rowTabHistory'
import {useTranslation} from "react-i18next";

export const RowOrderExtra = ({active, data, arrDetail,onActionCancelForControlCOD, setOpenEditCODModal, status, ...props}) => {
  const [activeTab, setActiveTab] = useState(
    ForControlCOD_TABLE_ROW_EXTRA_TABS[0]?.value,
  )
      ...
      {active && (
        <div className="row-delivery-extra__container">
          <div className="row-delivery-extra__tabs">
            {ForControlCOD_TABLE_ROW_EXTRA_TABS.map(item => (
              <div
                key={item.id}
          ...
          <div className="row-delivery-extra__content">
            {activeTab === 'detailForControlCOD' && <RowTabDetailForControlCOD data={data} status={status} onActionCancelForControlCOD={onActionCancelForControlCOD} setOpenEditCODModal={setOpenEditCODModal} />}
