...
import ConfirmModalForControlCOD from '../../ConfirmModal'
import {StyledOrderExport} from './_styled'
import {useTranslation} from "react-i18next";

export const ForControlCODExport = ({data, ...props}) => {
  ...
  return (
    ...
    >
      <ConfirmModalForControlCOD confirm={confirm} setConfirm={setConfirm} />
      <div className="order-export__container">
        <div className="order-export__header">
          ...
          <Text as="h5" fontSize={18} fontWeight={700} lineHeight={22}>
            {numberWithCommas(data?.data?.total || 0)} {t("order_")}
          </Text>
...
const Item = ({
  ...
  const handleExport = async () => {
    ...
    const response = await sendRequestAuth(
      'get',
      `${config.API}/cod/cod-details-export${queryString}`,
      null,
    ...
    >
      ...
      <div className="order-export__info">
        <Text as="b" style={{display: 'block'}}>
          Xuất dữ liệu từ {numberWithCommas(index * ORDER_LIMIT_EXPORT + 1)} -{' '}
          {numberWithCommas(
            Math.min(
