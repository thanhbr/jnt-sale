...
import {ORDER_LIMIT_EXPORT} from 'Pages/Report/Warehouse/Pages/Import/interfaces/_constants'
import {ORDER_ICONS} from 'Pages/Report/Warehouse/Pages/Import/interfaces/_icons'
import {numberWithCommas} from 'Pages/Report/Warehouse/Pages/Import/utils/string'
import {useRef} from 'react'
import {useEffect} from 'react'
import {useState} from 'react'
import {StyledImportExport} from './_styled'

export const ImportExport = ({data, ...props}) => {
  const buttonNumber = Math.ceil(data?.data?.total / ORDER_LIMIT_EXPORT)

  return (
    <StyledImportExport {...props}>
      <div className="import-export__container">
        <div className="import-export__header">
          <span className="import-export__banner">{ORDER_ICONS.xlsx}</span>
        ...
        <div className="import-export__body">
          <div className="import-export__list common-scrollbar">
        ...
        <div className="import-export__footer">
          <Button
            appearance="ghost"
            size="sm"
            style={{minWidth: 110}}
        ...
        <div className="import-export__bg">
          <img src="/img/import/export-bg.png" alt="bg" />
        </div>
      </div>
    </StyledImportExport>
  )
...
const Item = ({data, exportData, isOnly, onClose}) => {
  ...
  const handleExport = async () => {
    ...
    const response = await sendRequestAuth(
      'get',
      `${config.API}/import/export-imports-xlsx${queryString}`,
    )
    ...
    >
      ...
      <div className="import-export__action">
        ...
        {status === 2 && (
          <>
            <CircularProgress
              className="import-export__loading"
              size={28}
              thickness={4}
            />
            <i className="import-export__cancel">{ORDER_ICONS.xExcel}</i>
          </>
        )}
        {![1, 2].includes(status) && (
          <span className="import-export__download">{ORDER_ICONS.export}</span>
        )}
      </div>
    </div>
  )
