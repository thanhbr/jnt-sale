...
const useLocationFilterForm = () => {
  ...
  const handleLocationKeywordChange = data => {
    let foundLocationId = data?.id
    if (!foundLocationId) {
      const foundLocation = locationListOrigin.find(
        item => item.name === data?.value.trim(),
      )
      foundLocationId = foundLocation?.value || ''
    }
...
// ===== ===== ===== ===== =====
  ...
  const applyLocationOtherFilter = async () => {
    const collection = {
      ...queries,
      start_date:
    ...
    fetchLocationByFilter(collection, { forceLoading: true })
  }

  const fetchLocationByFilter = async (qs, opt) => {
    ...
    const response = await Promise.all([
      sendRequestAuth('get', `${config.API}/report/sales/area-sales-report${queryString}`),
  ...
  return {
    ...
    functions: {
      ...
      fetchLocationWithCurrentFilter: () =>
        fetchLocationByFilter(
          {
            ...queries,
            start: table.pagination.active * table.pagination.amount,
...
export default useLocationFilterForm
