...
import {orderActions, OrderOriginReducer} from '../provider/_reducer'
import {OrderOriginInitialState} from '../provider/initState'
import {useParams, useSearchParams} from 'react-router-dom'

const useOrderOrigin = () => {
  const [state, dispatch] = useReducer(OrderOriginReducer, OrderOriginInitialState)
  ...
  const handleOriginFetch = async dateTimeValue => {
    ...
    const response = await Promise.all([
      sendRequestAuth(
        'get',
        `${config.API}/report/sales/order-origin-sales-report?order_origin_id=&keyword=${querySearch}&start_date=${startDate}&end_date=${endDate}&date_type=received`,
      ),
      sendRequestAuth(
        'get',
        `${config.API}/report/sales/order-origin-total-sales-report?order_origin_id=&keyword=${querySearch}&start_date=${startDate}&end_date=${endDate}&date_type=received`,
      )
    ])
