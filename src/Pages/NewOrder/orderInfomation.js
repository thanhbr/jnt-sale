...
export default function OrderInfomation({...props}) {
    ...
    getData(urlOrigin)
      .then(res => {
        if (res && res.data && res.data.success) {
          ...
          if (!selectOr.value) {
            dispatch({type: 'UPDATE_ORGIGIN_NEW_ORDER', payload: newData[0]})
          }
        }
      })
      .catch(err => {
    ...
    try {
      ...
      try {
        ...
        postData(url, data)
          ...
          .catch(error => {
            UposLogFunc(`ERROR CREATE ORDER 1 ${error.message}`)
            console.log(`error ============>${error}`)
          })
      } catch (error) {
        console.log(`error ============>${error}`)
        UposLogFunc(`ERROR CREATE ORDER 2 ${error.message}`)
      }
    } catch (error) {
      UposLogFunc(`ERROR CREATE ORDER 3 ${error.message}`)
      return null
    }
  }
  ...
  return (
    <div className="client-info-wrapper bg-order-cmp order-info-wrapper new-order-order-infomation">
      <div className="create-new-order-title-wrapper">
        <div className="upos-text-roboto-medium-500 upos-text-indygo-dye">{t('order_information')}</div>
      </div>
