...
const usePaymentManagementFilter = () => {
    ...
    const formatDate = (date)=>{
        ...
        const slitDate = format?.split('/')
        const slitTime = slitDate[2].split(' ')
        return  `${slitTime[0]}-${slitDate[1]}-${slitDate[0]} ${slitTime[1]}`
    }

    const queries = {
        keyword: filter?.keyword || '',
        start_date:filter?.dateTime?.start? formatDate(filter?.dateTime?.start) : '',
        end_date:filter?.dateTime?.end? formatDate(filter?.dateTime?.end) : '',
    ...
    const fetchReceipt = async (qs) => {
        pageDispatch({type: PaymentManagementActions?.TABLE_DISPLAY_DATA_UPDATE, payload: {list: [], loading: true}})
        ...
        if (response?.data?.success) {
            ...
            pageDispatch({type: PaymentManagementActions?.TABLE_PAGINATION_UPDATE,
    ...
    const handleChangeSearch = e => {
        pageDispatch({type: PaymentManagementActions.FILTER_CHANGE_SEARCH_KEYWORD, payload: e})
        debounceChangeSearch(e)
    }
    ...
    const handleDateTimeChange = data => {
        pageDispatch({
            type: PaymentManagementActions.FILTER_DATE_TIME_UPDATE,
            payload: {
                end: data.value[1],
                start: data.value[0],
                type: data.category,
    ...
    const handleGroupSubmitterChange = cate => {
        pageDispatch({
            type: PaymentManagementActions.FILTER_OTHER_GROUP_SUBMITTER_UPDATE_VALUE,
            payload: cate
        })
    }
    ...
    const handleGroupSubmitterKeywordChange = cate => {
        pageDispatch({
            type: PaymentManagementActions.FILTER_OTHER_GROUP_SUBMITTER_CHANGE_KEYWORD,
            payload: cate?.value
        })
        ...
        pageDispatch({
            type: PaymentManagementActions.FILTER_OTHER_GROUP_SUBMITTER_UPDATE_LIST,
    ...
    const handleChangePaymentMethod = data => {
        ...
        pageDispatch({
            type: PaymentManagementActions.FILTER_OTHER_PAYMENT_METHOD_UPDATE_VALUE,
            payload: {list: paymentMethodListData, value: result}
        })
        if(paymentMethodListData?.length === 0) {
            pageDispatch({
                type: PaymentManagementActions.FILTER_OTHER_PAYMENT_METHOD_TAB_UPDATE,
                payload: {
    ...
    const handlePaymentMethodKeywordChange = cate => {
        pageDispatch({
            type: PaymentManagementActions.FILTER_OTHER_PAYMENT_METHOD_CHANGE_KEYWORD,
            payload: cate?.value
        })
        const formatDataValue = cate?.value
            ? removeAcent(cate?.value?.trim()?.toLowerCase())
    ...
    const handleChangeEmployeeCreate = data => {
        ...
        pageDispatch({
            type: PaymentManagementActions.FILTER_OTHER_EMPLOYEE_CREATE_UPDATE_VALUE,
            payload: {list: employeeCreateListData, value: result}
        })
        if(employeeCreateListData?.length === 0) {
            pageDispatch({
                type: PaymentManagementActions.FILTER_OTHER_EMPLOYEE_CREATE_TAB_UPDATE,
                payload: {
    ...
    const handleEmployeeCreateKeywordChange = cate => {
        pageDispatch({
            type: PaymentManagementActions.FILTER_OTHER_EMPLOYEE_CREATE_CHANGE_KEYWORD,
            payload: cate?.value
        })
        const formatDataValue = cate?.value
            ? removeAcent(cate?.value?.trim()?.toLowerCase())
    ...
    const handleChangeTypeReceipt = data => {
        ...
        pageDispatch({
            type: PaymentManagementActions.FILTER_OTHER_TYPE_RECEIPT_UPDATE_VALUE,
            payload: {list: typeReceiptListData, value: result}
        })
        if(typeReceiptListData?.length === 0) {
            pageDispatch({
                type: PaymentManagementActions.FILTER_OTHER_TYPE_RECEIPT_TAB_UPDATE,
                payload: {
    ...
    const handleTypeReceiptKeywordChange = cate => {
        pageDispatch({
            type: PaymentManagementActions.FILTER_OTHER_TYPE_RECEIPT_CHANGE_KEYWORD,
            payload: cate?.value
        })
        const formatDataValue = cate?.value
            ? removeAcent(cate?.value?.trim()?.toLowerCase())
    ...
    const applyOrderOtherFilter = _ => {
        ...
        // Datetime
        pageDispatch({
            type: PaymentManagementActions.FILTER_ACTIVE_DATE_TIME_UPDATE,
            payload: {
        ...
        // Group submitter
        pageDispatch({
            type: PaymentManagementActions.FILTER_ACTIVE_OTHER_GROUP_SUBMITTER_UPDATE,
            payload: filter?.groupSubmitter?.value
        })
        // Payment method
        pageDispatch({
            type: PaymentManagementActions.FILTER_ACTIVE_OTHER_PAYMENT_METHOD_UPDATE,
            payload: filter?.paymentMethod?.value
        })
        // Employee create
        pageDispatch({
            type: PaymentManagementActions.FILTER_ACTIVE_OTHER_EMPLOYEE_CREATE_UPDATE,
            payload: filter?.employeeCreate?.value
        })
        // Type receipt
        pageDispatch({
            type: PaymentManagementActions.FILTER_ACTIVE_OTHER_TYPE_RECEIPT_UPDATE,
    ...
    const filterTagDelete = type => {
        switch (type) {
            case 'dateTime':
                ...
                pageDispatch({
                    type: PaymentManagementActions.FILTER_DATE_TIME_TRIGGER_UPDATE,
    ...
    const handleChangeStatus = status => {
        pageDispatch({
            type: PaymentManagementActions.FILTER_ACTIVE_OTHER_STATUS,
            payload:
...
export default usePaymentManagementFilter;