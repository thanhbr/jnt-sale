...
const usePaymentManagementRow = data => {
    ...
    const fetchRowDetail = async () => {
        const response = await sendRequestAuth(
            'get',
            `${config.API}/cashbook/payments/detail/${data.id}`,
        )
    ...
    const handlePrint = async _ => {
        const response = await sendRequestAuth(
            'get',
            `${config.API}/cashbook/payments/print/${data.id}`,
        )
        if (response?.data?.success) {
    ...
    const handleEditDesc = _ => {
        ...
        pageDispatch({type: PaymentManagementActions.UPDATE_MODAL_EDIT_DESCRIPTION, payload: pageState?.table?.detail?.active?.description || ''})
    }

    const handleCancelReceipt = _ => pageDispatch({
        type: PaymentManagementActions.TOGGLE_MODAL_CANCEL_RECEIPT,
        payload: {
...
export default usePaymentManagementRow;