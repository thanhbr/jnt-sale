...
import {WAREHOUSE_TRANSFER_TABLE_ROW_MENU_POPOVER} from 'Pages/WareHouseTransfer/interfaces/_constants'
...
export const RowMenuPopover = ({onActionClick, ...props}) => {
  const [actionList, setActionList] = useState(
    WAREHOUSE_TRANSFER_TABLE_ROW_MENU_POPOVER,
  )
  ...
  useEffect(() => {
    let actionListOrigin = [...WAREHOUSE_TRANSFER_TABLE_ROW_MENU_POPOVER]

    if (
      ['2', '22', '23', '3', '4', '5', '6', '19', '20', '17'].includes(
        props.item?.shipping_status_id,
      )
    ) {
      actionListOrigin = actionListOrigin.filter(
        i => ![1, 2, 3, 4, 5, 6].includes(i.id),
      )
    }

    if (
      [
        props.item?.down_cod_status === '0',
    ...
    props.item?.shipping_status_id,
    props.item?.down_cod_status,
    props.item?.partner_id,
    props.item?.partsign,
    props.item?.cod,
  ])
  ...
  return (
    <>
      <StyledIconToggle
        ...
        style={{
          cursor: 'pointer',
          display: actionList.length > 0 ? 'block' : 'none',
        }}
        onClick={handleClick}
      >
        {WAREHOUSE_TRANSFER_ICONS.manipulation}
      </StyledIconToggle>
      <Popover
        id={id}
          ...
          {actionList.map((item, key) => {
            return +item.id === 2 && +props.item?.shipping_status_id !== 1 ? (
              ''
                ...
                ['1', '8'].includes(props.item?.shipping_status_id) &&
                ['1'].includes(props.item?.partner_id)) ||
              +item.id !== 1 ? (
              <li
                key={item.id}
