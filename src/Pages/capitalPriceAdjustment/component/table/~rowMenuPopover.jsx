...
import {PRICE_ADJUSTMENT_TABLE_ROW_MENU_POPOVER} from "../../interfaces/_const";
  ...
  return (
    <>
      <StyledIconToggle
        // aria-describedby={id}
        variant="contained"
        style={{ cursor: 'pointer', position: 'absolute', right: 4 }}
        onClick={handleClick}
      >
        <div className={'common-icon'} data-active={open}>{PRICE_ADJUSTMENT_ICONS.manipulation}</div>
      </StyledIconToggle>
      <Popover
        id={id}
      ...
      >
        <StyledRowMenuPopover>
          {PRICE_ADJUSTMENT_TABLE_ROW_MENU_POPOVER.map((item, key) => (
            <li
              key={item.id}
