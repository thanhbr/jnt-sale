...
const useDelivery = () => {
  ...
  const filterQueries = {
    ...
    adminUser: filter.adminUser?.activeValue?.value || '',
    orderOrigin: filter.orderOrigin?.activeValue?.value || '',
  }

  const handleGetReportStatus = async (phoneList, displayListData, displayArrDetailsData) => {
  ...
  const handleOriginFetch = async dateTimeValue => {
    ...
    const response = await sendRequestAuth(
      'get',
      `${config.API}/order/partsign/list?keyword=&date_type=sended&start_date=${startDate}&end_date=${endDate}&shipping_partner=&user_id=&order_origin_id=&per_page=${table.pagination.amount}&start=${table.pagination.active}`,
    )
  ...
  const handlePaginationAmountChange = async n => {
    ...
    const response = await sendRequestAuth(
      'get',
      `${config.API}/order/partsign/list${queryStr}`,
    )
