...
import {RECEIPT_TABLE_THEAD_STATUS_FILTER_LIST} from "../../interfaces/contant";
...
const StatusFilterPopover = ({...props}) => {
  const {filter, methods} = useReceiptFilter()
  ...
  const shouldActiveIcon = !!(
    filter?.status?.length > 0 && filter?.status?.length < RECEIPT_TABLE_THEAD_STATUS_FILTER_LIST?.length
  )
  ...
  const handleReset = () => {
    ...
    handleChange(RECEIPT_TABLE_THEAD_STATUS_FILTER_LIST.map(item => item?.value))
  }
  ...
  return (
    <>
      ...
      >
        {shouldActiveIcon
          ? RECEIPT_ICONS.filterFunnel02_solid
          : RECEIPT_ICONS.filterFunnel02 }
      </i>
      {open && (
        <Popover
          id={id}
        ...
        >
          <StyledStatusFilterPopover {...props}>
            {RECEIPT_TABLE_THEAD_STATUS_FILTER_LIST.map(item => (
              <li
                key={item.id}
