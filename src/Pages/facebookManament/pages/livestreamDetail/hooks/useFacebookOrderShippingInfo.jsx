...
const useFacebookOrderShippingInfo = () => {
  ...
  const getShippingPartner = async () => {
    ...
    if (!!response?.data?.success) {
      ...
      const listPartner = transformData.filter(item => {
        if (item.is_default) {
          dispatch({
            type: facebookLiveStreamDetailActions.UPDATE_SHIPPING_PARTNER_INFO,
            payload: {
              id: item.id,
            },
          })
          dispatch({
            type: facebookLiveStreamDetailActions.SET_SHIPPING_PARTNER_DEFAULT,
            payload: item.id,
          })
          dispatch({
            type: facebookLiveStreamDetailActions.UPDATE_SHIPPING_PARTNER_SELECTED,
            payload: item.id,
          })
          dispatch({
            type: facebookLiveStreamDetailActions.SET_COLLAPSE_SHIPING,
            payload: [item.id],
          })
        }
      ...
      dispatch({
        type: facebookLiveStreamDetailActions.UPDATE_LIST_SHIPPING_PARTNER,
        payload: {
          list: listPartner,
  ...
  const fetchSomeData = async params => {
    ...
    if (!!response?.data?.success) {
      ...
      dispatch({
        type: facebookLiveStreamDetailActions.UPDATE_LIST_SHIPPING_FEE,
        payload: {
          list: partner,
        },
      })
      dispatch({
        type: facebookLiveStreamDetailActions.SET_SHIPPING_SERVICE,
        payload: services,
      })
      dispatch({ type: 'UPDATE_LOADING_FEE', payload: false })
    }
  ...
  const getFeeShippingDetail = async (data, opt = {}) => {
    ...
    if (!!response?.data?.success) {
      ...
      dispatch({
        type: facebookLiveStreamDetailActions.UPDATE_SHIPPING_FEE,
        payload: {
  ...
  const onResetShippingPartner = () => {
    ...
    if(!!state.shipping_status.value) {
      getFeeShipping(customerInfo.ward.value, {
        queries: {
          cityId: +customerInfo.province.value?.id,
          districtId: +customerInfo.district.value?.id,
          wardId: +customerInfo.ward.value?.id,
    ...
    } else if (customerInfo.ward.value?.id)
      getFeeShipping(customerInfo.ward.value, {
        queries: {
          districtId: customerInfo.district.value?.id,
          cityId: customerInfo.province.value?.value,
        },
      })
  }
  ...
  const onChangeCod = money => {
    ...
    dispatch({
      type: facebookLiveStreamDetailActions.UPDATE_SHIPPING_INFO,
      payload: {
        cod: money,
      },
    })
  ...
  const onDeliveryNoteChange = async (e = '') => {
    ...
    dispatch({
      type: facebookLiveStreamDetailActions.VALIDATE_EDIT_FORM_DELIVERY_NOTE,
  ...
  const handleCargoInsurranceValidate = (boo) => {
    ...
    dispatch({
      type: facebookLiveStreamDetailActions.SET_VALIDATE_INSURRANCE,
  ...
  const onChangeCargoInsurrance = (value) => {
    ...
    dispatch({
      type: facebookLiveStreamDetailActions.SHIPPING_INSURRANCE_UPDATE,
      payload: {
        active: value,
  ...
  const handleValidatePackageQuantity = (boo,position) => {
    ...
    dispatch({
      type: facebookLiveStreamDetailActions.SET_VALIDATE_SHIPPING_PARTNER,
  ...
  const handleDateTimeChange = data =>
    dispatch({
      type: facebookLiveStreamDetailActions.FORM_SHIPPING_DATETIME_UPDATE,
      payload: { ...data },
    })
  ...
  const handlePickUpStoreToggle = () => {
    ...
    dispatch({
      type: facebookLiveStreamDetailActions.FORM_PAYMENT_METHOD_TYPE_UPDATE,
      payload: {type: !shippingInfo.isStorePickUp ? 'after' : 'cod'},
    })

    dispatch({ type: facebookLiveStreamDetailActions.UPDATE_SHIPPING_INFO_PICK_UP_STORE })
  }
