...
import {FacebookAutoResponsesContext, FacebookOrdersContext} from '../provider/_context'
...
const useFacebookAutoResponses = () => {
    ...
    const getListPost = async (page_id) => {
        const response = await sendRequestAuth('get',
            `${config.API}/fb/post/list?page_id=${!!page_id && page_id.toString()}&keyword=&time&per_page=20&start=0`
        )
    ...
    const handlePageSelectedChange = data => {
        const find = pageSelectedValue.find(item => item.value === data.id)
        const pageSelectedListData =
            pageSelectedTab === 'checked'
                ? pageSelectedValue.filter(item => item.value !== data.id)
                : pageSelectedList
        const pageSelectedValueData = find
            ? pageSelectedValue.filter(item => item.value !== data.id)
            : [...pageSelectedValue, {name: data.value, value:data.id,avatar:data.avatar}]
        pageDispatch({
        ...
        if(pageSelectedValueData.length > 0){
            ...
            const id = pageState.filter.pageSelected?.listOrigin?.map(item=>item.value)
            getListPost(id)
        }
