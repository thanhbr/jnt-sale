...
export const UpdateStatusFailedModal = ({data, onClose, ...props}) => {
  return (
    <StyledUpdateStatusFailedModal {...props} onClick={onClose}>
      <div
        className="order-table-facebook__update-status-failed-modal__container"
      ...
      >
        ...
        {Array.isArray(data) && data.length > 0 && (
          <div className="order-table-facebook__update-status-failed-modal__content common-scrollbar">
            {data.map(item => (
              <div
                key={item.id}
                className="order-table-facebook__update-status-failed-modal__content-item"
              >
                {ORDER_ICONS.danger}
        ...
        <div className="order-table-facebook__update-status-failed-modal__footer">
          <Button
            appearance="ghost"
            size="sm"
            style={{minWidth: 110}}
...
const StyledUpdateStatusFailedModal = styled.div`
  ...
  .order-table-facebook__update-status-failed-modal {
    &__container {
      width: 700px;
      padding: 24px;
