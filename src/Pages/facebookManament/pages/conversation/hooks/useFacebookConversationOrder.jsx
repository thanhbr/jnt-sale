...
import { facebookConversationActions, facebookConversationActions as actions } from '../provider/_actions'
...
const useFacebookConversationOrder = () => {
  ...
  const pageId = state.detail?.conversation?.customer?.page_id || ''
  const postId = state.detail?.conversation?.customer?.post_id || ''
  const fbID = state.detail?.conversation?.customer?.from || ''
  ...
  const handleProductValidate = (boo) => {
    ...
    dispatch({
      type: actions.SET_VALIDATE_PRODUCT_INFOR,
  ...
  const handleCODValidate = (boo) => {
    ...
    dispatch({
      type: facebookConversationActions.SET_VALIDATE_COD,
      payload: cod,
    })
  }
  ...
  const handleWeightValidate = (boo) => {
    ...
    dispatch({
      type: facebookConversationActions.SET_VALIDATE_WEIGHT,
      payload: weight,
    })
  }
  ...
  const handleLengthChange = val => {
    if (val > 150){
      val = 150
      showAlert({
        content: "Chiều dài không được vượt quá 150cm.",
        type: 'danger',
        duration: 2000
      })
    }
  ...
  const handleWidthChange = val => {
    if (val > 150){
      val = 150
      showAlert({
        content: "Chiều rộng không được vượt quá 150cm.",
        type: 'danger',
        duration: 2000
      })
    }
  ...
  const handleHeightChange = val => {
    if (val > 150){
      val = 150
      showAlert({
        content: "Chiều cao không được vượt quá 150cm.",
        type: 'danger',
        duration: 2000
      })
    }
  ...
  const submitQueries = {
    ...
    shipping: !shippingInfo.isStorePickUp
      ? {
        cod: transformMoneyToSendRequest(shippingInfo.cod || ''),
  ...
  const handleSubmit = async opt => {
    if(shippingInfo?.cod == '' || shippingInfo?.weight == ''){
      handleCODValidate(shippingInfo?.cod == '' ? true : false)
      handleWeightValidate(shippingInfo?.weight == '' ? true : false)
    }else{
      if (debounceSubmit) {
