import useFacebookOrders from '../../hooks/useFacebookLivestream'
...
export const ListPage = () => {
  ...
  const nav = useNavigate()
    ...
    const showFallBack = () => {
        const shouldDisable = page?.list?.length <= 1

        const handleActivePage = (page_id) => {
          if (shouldDisable) return
          methods.handlePageChange(page_id)
        }

        return page?.list.map((item, key) =>
            <Tooltip title={item.page_name} placement={'right'}>
                <div className={'list-page__item'} 
                style={page.active.length == 1 && page.active[0] == item.page_id ? {cursor:'no-drop'} : {cursor:'pointer'}}
                onClick={() => handleActivePage(item.page_id)}>
