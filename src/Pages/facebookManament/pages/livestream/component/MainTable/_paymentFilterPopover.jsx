...
export const PaymentFilterPopover = ({...props}) => {
  const {payment} = useFacebookFilterForm()
        ...
        >
          <StyledPaymentFilterPopover {...props}>
            {ORDER_TABLE_THEAD_PAYMENT_FILTER_LIST.map(item => (
              <li
                key={item.id}
                className="livestream-table-facebook__payment-filter-popover__list-item"
            ...
            <li className="livestream-table-facebook__payment-filter-popover__footer">
              <Text
...
const StyledPaymentFilterPopover = styled.ul`
  ...
  .livestream-table-facebook__payment-filter-popover {
    &__list-item {
      height: 40px;
