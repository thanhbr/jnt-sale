...
const Index = ({dataRow }) => {
  ...
  const handleItemClick = type => {
    if(type === 1) functions.editPaymentMethod(dataRow)
    if(type === 2) functions.deleteItem(dataRow)
    handleClose()
  }
  return (
    <>
      <StyledIconToggle
        ...
        onClick={handleClick}
      >
        {PAYMENT_METHOD_ICONS.manipulation}
      </StyledIconToggle>
      {open && (
        <Popover
          id={id}
        ...
        >
          <StyledRowMenuPopover>
            {actionList.map(item => {
              if(item?.id === '2' && dataRow?.is_active === '1' || item?.id === '2' && dataRow?.is_default === '1') {
                return ''
              } else {
                return (
                  <li
