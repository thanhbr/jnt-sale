...
function RenderCOD({...props}) {
  const {cod, value, changeCod, messCOD, dispatch} = props
  const {t} = useTranslation()
  return (
    <>
      <div
...
function RenderInsurrance({...props}) {
  const {value, messInssurance} = props
  const {t} = useTranslation()
  return (
    <>
...
function RenderInsurranceValue({...props}) {
  ...
  return (
    <>
      <input
        value={InssuranceValue}
        onChange={e => {
          const text = e.target.value
          changeInssuranceValue(text)
          if (timeout) clearTimeout(timeout)
          timeout = setTimeout(() => {
            dispatch({
              type: 'CHANGE_DETAIL_SHIPPING_NEW_ORDER',
        ...
        className="upos-input upos-text detail-tranport-input input-boderfocus-indent insurrance-input show-unit-input"
      />
      <div className="upos-error-text upos-animation-opacity upos-text">
...
function RenderPaymentSide({...props}) {
  ...
  return (
    <>
      <div className="label-input payment-detail-tran person-payment">Người trả phí</div>
      <CheckboxRadio
        list={
          value !== '1'
            ? [
...
function RenderPickDate({...props}) {
  ...
  const Globalstate = useGlobalContext()[0]
...
const CheckboxRadio = ({...props}) => {
  ...
  try {
    ...
    const caculateSelect = () => {
      ...
      if ((state.new_order.shipping_info.partner[shipping_id] || {})[params]) {
        const stagSelect = (state.new_order.shipping_info.partner[
          shipping_id
        ] || {})[params]
        return stagSelect
      }
      const stagSelect = list[0]
      dispatch({
        type: 'CHANGE_DETAIL_SHIPPING_NEW_ORDER',
        payload: {
          field: shipping_id,
    ...
    <Grid container className="tranport-detail-wrapper">
      ...
      <Grid className="right-trans-detail" item xs={12} sm={12} md={6}>
        ...
        <RenderCOD
          ...
          dispatch={dispatch}
        />
        <RenderInsurrance value={value} messInssurance={messInssurance} />
  ...
  return (
    <div className="TranportInfomation bg-order-cmp">
      <div className="prod-info-header">
        <div className="prod-info-label">Thông tin vận chuyển</div>
        ...
        <>
          ...
          arrListPartner.length > 0 ? (
            arrListPartner.map((value, index) => {
              ...
              const {id} = item
              // console.log("id shipping partner ==================== " + id);
              return (
                <RenderRow
                  key={uuidv4()}
                  item={item}
